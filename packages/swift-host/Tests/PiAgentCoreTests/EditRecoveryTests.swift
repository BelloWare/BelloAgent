...
final class EditRecoveryTests: XCTestCase {
    ...
    func testEditingKeptTurnPreservesCompactionSummaryInTimelineAndContext() async throws {
        ...
        // Pi's cut at the second turn's start keeps that whole turn: "second" and its answer are 6 tokens.
        let session = try AgentSession(id: "edit", profile: profile, apiKey: "k", cwd: root, directory: state, readOnly: true, resources: resources, client: client, tools: RecordingTools(), traces: traces, autoCompaction: false, compactionPolicy: { var policy = CompactionPolicy(); policy.keepRecentTokens = 6; return policy }())
