...
extension PayloadArchive {
    /// Actor-isolated, indexed metadata queries. HTTP bodies, authentication and
    /// message links are deliberately absent. One primary-key attempt is one
    /// observation regardless of tool rounds, message attribution or refreshes.
    func menuBarMetrics(period: MenuBarPeriod, until: Date = Date(), offset: Int = 0) throws -> MenuBarSnapshot {
        try usageMetrics(period: period, until: until, offset: offset)
    }

    /// A session's retained history includes every HTTP attempt of its own tool
    /// rounds and compactions. Inherited message links never import another
    /// session's requests, and workspace identity always participates in scope.
    func sessionMetrics(sessionID: String, workspaceID: String, until: Date = Date(), offset: Int = 0) throws -> MenuBarSnapshot {
        ...
        return try usageMetrics(period: .retained, until: until, offset: offset, sessionID: sessionID, workspaceID: workspaceID)
    }

    private func usageMetrics(period: MenuBarPeriod, until: Date, offset: Int, sessionID: String? = nil, workspaceID: String? = nil) throws -> MenuBarSnapshot {
        ...
        let summary = try db.rows("""
        SELECT \(Self.gatewayAggregateSQL),\(Self.historicalOutputRateSQL),COUNT(DISTINCT workspace) AS workspaces,
        ...
        let sessionCount = Int(try db.rows("SELECT COUNT(*) AS n FROM (SELECT workspace,session FROM attempts WHERE \(selected) GROUP BY workspace,session)", values).first?["n"]?.number ?? 0)
        var totals = Self.gatewayTotals(summary)
        ...
        var counts = DashboardCounts(dispatched: totals.requests)
        ...
        \(normalized)
        SELECT api,alias,resolved_model,resolution_status,\(Self.gatewayAggregateSQL),\(Self.historicalOutputRateSQL)
        ...
        var models = try rows.map { row -> MenuBarModelDistribution in
            ...
            let gateway = Self.gatewayTotals(row)
            let share: Double?
            ...
            return MenuBarModelDistribution(api: api, requestedAlias: alias, resolvedModel: row["resolved_model"]?.string, identityStatus: status, gateway: gateway, allRequests: totals.requests,
                                            historicalRate: Self.historicalOutputRate(row), costShare: share)
        }
        if !models.isEmpty {
        ...
        try Task.checkCancellation()
        let from = period.start(until: until) ?? summary["first_wall"]?.double.map { Date(timeIntervalSince1970: $0) }
        var buckets: [MenuBarBucket] = []
        if let from, until.timeIntervalSince(from) >= 1 {
            ...
            for row in try db.rows("SELECT CAST((wall-?)/? AS INTEGER) AS bucket,\(Self.gatewayAggregateSQL),\(Self.historicalOutputRateSQL) FROM attempts WHERE \(selected) GROUP BY bucket", bucketArgs + values) {
                ...
                buckets[index].gateway = Self.gatewayTotals(row); buckets[index].historicalRate = Self.historicalOutputRate(row)
            }
        }
        return MenuBarSnapshot(period: period, from: from, until: until, counts: counts, gateway: totals,
                               ...
                               models: models, modelGroups: groupCount, offset: offset, historicalRate: Self.historicalOutputRate(summary), buckets: buckets)
    }
