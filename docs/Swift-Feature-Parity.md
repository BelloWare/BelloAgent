...
| Compaction | Complete-boundary summary and recent turns, native resume; composer strip shows progress and the latest successful summary from authoritative active context. One summary request per compaction: a split turn's start is summarized in it (pi sends a second request), and nothing is chunked | Fast/background completion and historical baselines are independent of transcript scrollback. Failed/cancelled attempts cannot reuse an old success; abandoned branches cannot supply the notice. No recursive overflow recovery or automatic retry. A history one request cannot hold beside the summary's room (after a switch to a smaller window) is refused before anything is sent. |
