...
`f00b69bcba3669a857a021618719f98377a528ff278a783fbc3f1726575b4fe1`.
The product page, identical update feeds and downloaded hash/Ed25519 verification
passed at **2026-09-25 14:39:14 UTC**. Cloudflare check `108116512846` succeeded.
Compaction reserves the larger ordinary/summary output requirement, uses the
uploaded checkpoint prompt, requires positive completion evidence and rejects
provider-hosted tool calls. Server truncation is explicitly disabled.
All 449 helper tests, 112 gallery screenshots and transport/script checks passed.
...
SHA-256 `161128eea9dc46f378d6100e7b53b8fea0bfc286c37fe0d6e61b8716730d2227`. The public product page,
identical update feeds and downloaded hash/Ed25519 verification passed at
**2026-09-24 19:41:00 UTC**. A compaction is one summary request: a split turn's start is summarized with the history,
and nothing is chunked. A history one request cannot hold is refused before anything is sent,
with the context unchanged. The gate passed 1,615 native tests, 435 helper
tests and the script checks; the gallery's compaction scene and one load-sensitive wire test were
fixed after it and pass (112 screenshots; 3 of 3).
See the [0.1.99 validation record](validation/Bello-Agent-0.1.99-2026-09-25.md).

**Bello Agent 0.1.98/build 102 is a historical verified release** at
[belloware.com](https://belloware.com/bello-agent.html), with source at
[Git tag v0.1.98](https://github.com/BelloWare/BelloAgent/tree/v0.1.98) and website commit
`ebbd7f5`. The signed/notarized DMG is **10,855,712 bytes (10.35 MiB)**;
SHA-256 `c76094273c2a92c9f47f64761cda2790e7c4ccb3516415a218624cc79bd18296`. The public product page,
identical update feeds and downloaded hash/Ed25519 verification passed at
**2026-09-24 16:54:57 UTC**. Each rendered reply is one TextKit text: a selection runs across its paragraphs, lists,
code and tables, and a copy is the text it covers. A streaming reply no longer rises over its
header between the row's measurements, and typing into a reply goes to the composer. The gate passed 1,616 native tests, 112 gallery
screenshots and the script checks with 0 failures; of 434 helper tests, one read its fixture's ready file half written,
and passes since the fixtures write it whole.
See the [0.1.98 validation record](validation/Bello-Agent-0.1.98-2026-09-25.md).

**Bello Agent 0.1.97/build 101 is a historical verified release** at
[belloware.com](https://belloware.com/bello-agent.html), with source at
[Git tag v0.1.97](https://github.com/BelloWare/BelloAgent/tree/v0.1.97) and website commit
`024bdd6`. The signed/notarized DMG is **10,848,082 bytes (10.35 MiB)**;
SHA-256 `98a9971df7fa8f8c61f8294122b29b8c41b23639532be3373caea16c09ecf3eb`. The public product page,
identical update feeds and downloaded hash/Ed25519 verification passed at
**2026-09-24 14:38:05 UTC**. The footer keeps one form through a run; the composer sizes from its text without lag;
a live chat fills a gap instead of stopping behind the newer edge; the Git panel finishes
refreshing while a build writes; and the helper lets go of memory a long day kept. The gate passed 1,611 native tests, 112 gallery screenshots, 434 helper
tests and the script checks with 0 failures, after two fixes its first run prompted. See the [0.1.97 validation record](validation/Bello-Agent-0.1.97-2026-09-24.md).

**Bello Agent 0.1.96/build 100 is a historical verified release** at
[belloware.com](https://belloware.com/bello-agent.html), with source at
[Git tag v0.1.96](https://github.com/BelloWare/BelloAgent/tree/v0.1.96) and website commit
`ea3c6879612225e8e4dd88e2a10472827d242808`. The signed/notarized DMG is **10,830,210 bytes (10.33 MiB)**;
SHA-256 `716cae0c0392941189f37601220183a41e3d6159753dbfb70f29b415e249acf6`. The public product page,
identical update feeds and downloaded hash/Ed25519 verification passed at
**2026-09-24 12:04:07 UTC**. Messages show as typed and replies switch to their source; the slash list no longer
flickers; model requests reuse their connection; side chats reuse the main chat's prompt cache;
the Inspector expands texts in place; and a performance, flicker and bug sprint landed. The gate
passed 1,602 native tests, 112 gallery screenshots, 433 helper tests and the script checks. Its two
parallel-lane failures were a test that needed its own fix and a reverted change, and both classes
passed afterwards. See the [0.1.96 validation record](validation/Bello-Agent-0.1.96-2026-09-24.md).

**Bello Agent 0.1.95/build 99 is a historical verified release** at
[belloware.com](https://belloware.com/bello-agent.html), with source at
[Git tag v0.1.95](https://github.com/BelloWare/BelloAgent/tree/v0.1.95) and website commit
`293b9bf8eb6ec73530ca37c951d9e3a79be7fb07`. The signed/notarized DMG is **10,720,890 bytes (10.22 MiB)**;
SHA-256 `efeee3e07362f365534e50eca53cb8ae0a2ae752ed6bbba87ca1c5e8740ffe37`. The public product page,
identical update feeds and downloaded hash/Ed25519 verification passed at
**2026-09-24 09:26:56 UTC**. ⌘↩ steers a running chat again: the Conversation menu had taken the key for "Send /
Queue Follow-up". The session's usage pill splits its tokens into uncached input, cached input
and output with its reasoning, and its cache hit has two decimal places. The full gate passed:
1,559 native tests, 100 gallery screenshots, 423 helper tests and the wire, concurrent, acceptance
and Python checks. See the
[0.1.95 validation record](validation/Bello-Agent-0.1.95-2026-09-24.md).

**Bello Agent 0.1.94/build 98 is a historical verified release** at
[belloware.com](https://belloware.com/bello-agent.html), with source at
...
SHA-256 `a2ada7728ab859301b9acb2c5357c75899d94fc6c3f916d3835cb1bf8fb58520`. The public product page,
identical update feeds and downloaded hash/Ed25519 verification passed at
**2026-09-24 01:45:32 UTC**. Summaries of histories too long for one request are no longer held to 13,107 tokens: each
chunk leaves a quarter of the window, within the model's limit. 412 helper tests, the wire,
concurrent and acceptance scripts, and the app's compaction, cost-limit and wire-contract classes
passed; the full gate ran on 0.1.91's code. See the
[0.1.92 validation record](validation/Bello-Agent-0.1.92-2026-09-24.md).

**Bello Agent 0.1.91/build 95 is a historical verified release** at
[belloware.com](https://belloware.com/bello-agent.html), with source at
[Git tag v0.1.91](https://github.com/BelloWare/BelloAgent/tree/v0.1.91) and website commit
`3ef6b6bcad809614f7d50a5f004295037b4cadc0`. The signed/notarized DMG is **10,569,935 bytes (10.08 MiB)**;
SHA-256 `231bd273a31f78e1089bf6349216248e1cc2ea752c97c766cc38e65db146cc61`. The public product page,
identical update feeds and downloaded hash/Ed25519 verification passed at
**2026-09-24 01:14:22 UTC**. Compaction no longer loops: summaries carry the model's own output limit, and a
compaction that fails or frees no room stops the run with a clear message. The release gate ran
on this code (two stale tests then fixed and rerun green), with the gallery, 411 helper tests, 29
wire tests and 55 script tests. See the
[0.1.91 validation record](validation/Bello-Agent-0.1.91-2026-09-24.md).

**Bello Agent 0.1.90/build 94 is a historical verified release** at
[belloware.com](https://belloware.com/bello-agent.html), with source at
...
SHA-256 `04747e794b6ad8b979b92b71d773b707a7c761ee844209e71cb8a616a8a2f268`. The public product page,
identical update feeds and downloaded hash/Ed25519 verification passed at
**2026-09-23 14:50:29 UTC**. Turn info shows each turn's input, output and models from every request, including
turns the auto router spread across models, and says what is missing when some requests did not
report. Compaction follows pi, with no source-size limit. **1,519 native tests passed** (0 failures),
with the gallery, 364 helper tests and 27 wire tests. See the
[0.1.88 validation record](validation/Bello-Agent-0.1.88-2026-09-23.md).

**Bello Agent 0.1.87/build 91 is a historical verified release** at
[belloware.com](https://belloware.com/bello-agent.html), with source at
[Git tag v0.1.87](https://github.com/BelloWare/BelloAgent/tree/v0.1.87) and website commit
`e3bb95647152ba875db831e8903786bcdc6013f5`. The signed/notarized DMG is **10,246,118 bytes (9.77 MiB)**;
SHA-256 `b52be05e7987ac6bf5ff6639021240c5edaf49a12441e62d719ab13e47b77a8b`. The public product page,
identical update feeds and downloaded hash/Ed25519 verification passed at
**2026-09-23 12:07:06 UTC**. Messages appear the instant Return is pressed, skills render inline,
the context meter counts as pi does, and drafts, runs and reading positions survive quit and
relaunch. **1,506 native tests** ran (one stale test restated and rerun green), with the gallery,
357 helper tests and 27 wire tests. See the
[0.1.87 validation record](validation/Bello-Agent-0.1.87-2026-09-23.md).

**Bello Agent 0.1.86/build 90 is a historical verified release** at
[belloware.com](https://belloware.com/bello-agent.html), with source at
[Git tag v0.1.86](https://github.com/BelloWare/BelloAgent/tree/v0.1.86) and website commit
`014278499c049867c8ad7f5c6bf3bb69aab3e446`. The signed/notarized DMG is **9,973,254 bytes (9.51 MiB)**;
SHA-256 `8049f6089c609fdf0ab349cb96aba39081c0ee6188028f1ca9758a3d25b999b7`. The public product page,
identical update feeds and downloaded hash/Ed25519 verification passed at
**2026-09-23 09:49:26 UTC**. Tokens per second is the standard decode speed (output tokens after
the first over the first to last generated token), the session pills open charts of where the time
and the tokens went, and a relaunch reopens the chat the reader had open. **1,438 native tests
passed** (0 failures), with the gallery, 344 helper tests and 27 wire tests. See the
[0.1.86 validation record](validation/Bello-Agent-0.1.86-2026-09-23.md).

**Bello Agent 0.1.85/build 89 is a historical verified release** at
[belloware.com](https://belloware.com/bello-agent.html), with source at
...
SHA-256 `03609f740f43f3213ada0b42ad3014eda2f6b86fd00f120ce9dc2bebd0b2eb16`. The public product page,
identical update feeds and downloaded hash/Ed25519 verification passed at
**2026-09-22 15:29:58 UTC**, after Cloudflare check **106813441104** succeeded.
Five reproduced lifecycle cases are fixed: skipped final metrics, stale terminal
labels, previous-execution request payloads, unchanged-poll disconnect recovery
and retired-session connection warnings. **71 native tests passed**, including
the local mock-gateway reopen flow; two optional visual tests were skipped.
See the [0.1.83 validation record](validation/Bello-Agent-0.1.83-2026-09-22.md)
for before/after evidence and the Xcode result-bundle reporting limitation.

**Bello Agent 0.1.82/build 86 is a historical verified release** at
[belloware.com](https://belloware.com/bello-agent.html), with source at
