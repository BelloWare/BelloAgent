...
model, cost and Info/Copy above duration and two thin token percentage bars.
Matching model names appear once. Model pairs come from the same retained
request, not the session's current picker; the latest pair follows dispatch time.
Info and copied details retain the original requested and response model names.
Input splits into cached and uncached tokens; output splits into reasoning and
other output. Shares require matching reported coverage and never double-count
subsets. Zero and unknown remain distinct. AI/tool time describes recorded phase
...
call's card, exposed reasoning, a compaction note, a whole response folded to
one line) is conversation state, not view state (0.1.59). `TranscriptDisclosure`, owned by the session's display,
...
between. A fold that spans several rows — a whole response — is recorded the
same way, and the conversation then reconciles its page so every row of that
response reaches its new height in one pass rather than each on its own. The shared geometry cache is keyed by that value, so a height measured
