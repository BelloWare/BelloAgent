...
SwiftUI/AppKit application, native composers, SQLite desktop metadata
    WKWebView + existing React transcript (presentation only)
                      |
...
`HostSupervisor.swift` launches `Contents/Helpers/pi-native-host`. Manifest/handshake identify engine `swift`, version `1.0.0`, protocol major 1 and capabilities. The transport remains local pipes with off-main IO. No credentials or raw debug bodies go to the transcript webview.
...
`build-bundle.py` builds the Swift release helper and React assets, stages Helpers/pi-native-host plus manifest/NOTICE and Transcript, and does not copy Node/node_modules. `sign-host.py` signs the helper without Node JIT entitlements. Existing smoke entry delegates to `smoke-native-bundle.py`. Keep the old TypeScript source as a development reference only.
...
remain mounted; a native visibility/focus guard hides their AppKit/WebKit
...
within smaller windows. Reduce Motion suppresses report/navigation animations.
...
and React message badges show totals and separate cache/token coverage. Only
bounded numeric projections and allowlisted model-identity summaries cross into
the webview, not raw headers or bodies. Assistant status lines show one reported
...
the tail. Up to eight hidden chats keep their pages in memory. A new chat or
...
its environment. Hiding it returns focus to the composer, and opening a chat
